...
設定画面の先頭に団名・所在地を表示。以下の項目を一覧表示し、各項目は詳細画面へ遷移。
...
| バージョン情報 | （画面内に表示、遷移なし） |
| データをすべて削除 | 二段階確認ダイアログ後に全テーブルを DELETE・SharedPreferences をクリアしてオンボーディングへ遷移 |
...
- 追加FAB・編集アイコン・削除アイコン
- フォーム：氏名は「氏」「名」の2フィールド（横並び・両方必須）、性別はラジオボタン（男性・女性）。新規追加時のデフォルトは「男性」
- 保存はAppBar右上の保存アイコン（`Icons.save_outlined`）。保存中はインジケーター表示
- 編集画面を抜ける際、未保存変更がある場合は確認ダイアログを表示
...
| status | TEXT DEFAULT 'planned' | planned / completed（確定）/ cancelled |
...
※ DB値 `completed` は UI表示ラベル「確定」に対応する。
...
| イベント | status = 'completed'（確定済み）または attendances が空でない |
...
- **条件**：期間内の確定済みイベント（DB値 `status = 'completed'`）すべてに `status = 'present'` で出席
- **欠席扱い**：出席者リストに登録されていない場合も欠席とみなす
- **確定済みイベントが0件の場合**：対象者なしとして空リストを返す
